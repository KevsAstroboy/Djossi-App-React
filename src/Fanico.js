...
const Fanico = () => {
  ...
  const [error, setError] = useState("Aucun fanico n'a été trouvé dans votre zone.");
  const [error1, setError1] = useState("Aucun fanico n'a été trouvé dans votre zone.");
  ...
  useEffect(() => {
    const fetchData = async () => {
      if (userData && userData.commune && userData.ville) {
        try {
          const response = await axios.get(`${apiUrl}api/fanico-user/${userData.commune}/${userData.ville}/`);
          ...
          // error = error.data.data;
            setError("Aucun fanico n'a été trouvé dans votre zone."); 
        }
      }
    };
  ...
  const handleSubmit = async (e) => {
    ...
    if (formData.commune && formData.ville) {
      ...
      const url = formData.optional ? `${apiUrl}api/fanico-user/${formData.commune}/${formData.ville}/${formData.optional}/`
                   : `${apiUrl}api/fanico-user/${formData.commune}/${formData.ville}/`;
      try {
        ...
        setError1("Aucun fanico n'a été trouvé dans votre zone.");
      }
    }
  };
