...
const Electricien = () => {
  ...
  const [error, setError] = useState("Aucun électricien n'a été trouvé dans votre zone.");
  const [error1, setError1] = useState("Aucun électricien n'a été trouvé dans votre zone.");
  ...
  useEffect(() => {
    const fetchData = async () => {
      if (userData && userData.commune && userData.ville) {
        try {
          const response = await axios.get(`${apiUrl}api/electricien-user/${userData.commune}/${userData.ville}/`);
          ...
          // error = error.data.data;
            setError("Aucun électricien n'a été trouvé dans votre zone."); 
            setLoading(false);
        }
      }
    };
  ...
  const handleSubmit = async (e) => {
    ...
    if (formData.commune && formData.ville) {
      ...
      const url = formData.optional ? `${apiUrl}api/electricien-user/${formData.commune}/${formData.ville}/${formData.optional}/`
                   : `${apiUrl}api/electricien-user/${formData.commune}/${formData.ville}/`;
      try {
        ...
        setError1("Aucun électricien n'a été trouvé dans votre zone.");
      }
    }
  };
