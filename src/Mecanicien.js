...
const Mecanicien = () => {
  ...
  const [error, setError] = useState("Aucun mécanicien n'a été trouvé dans votre zone.");
  const [error1, setError1] = useState("Aucun mécanicien n'a été trouvé dans votre zone.");
  ...
  useEffect(() => {
    const fetchData = async () => {
      if (userData && userData.commune && userData.ville) {
        try {
          const response = await axios.get(`${apiUrl}api/mecanicien-user/${userData.commune}/${userData.ville}/`);
  ...
  const handleSubmit = async (e) => {
    ...
    if (formData.commune && formData.ville) {
      ...
      const url = formData.optional ? `${apiUrl}api/mecanicien-user/${formData.commune}/${formData.ville}/${formData.optional}/`
                   : `${apiUrl}api/mecanicien-user/${formData.commune}/${formData.ville}/`;
      try {
