...
const Maquilleuse = () => {
  ...
  const [error, setError] = useState("Aucun(e) maquilleuses(eurs)) n'a été trouvé dans votre zone.");
  const [error1, setError1] = useState("Aucun(e) maquilleuses(eurs)) n'a été trouvé dans votre zone.");
  ...
  useEffect(() => {
    const fetchData = async () => {
      if (userData && userData.commune && userData.ville) {
        try {
          const response = await axios.get(`${apiUrl}api/makeup-user/${userData.commune}/${userData.ville}/`);
          ...
          // error = error.data.data;
            setError("Aucun(e) maquilleuses(eurs)) n'a été trouvé dans votre zone."); 
        }
      }
    };
  ...
  const handleSubmit = async (e) => {
    ...
    if (formData.commune && formData.ville) {
      ...
      const url = formData.optional ? `${apiUrl}api/makeup-user/${formData.commune}/${formData.ville}/${formData.optional}/`
                   : `${apiUrl}api/makeup-user/${formData.commune}/${formData.ville}/`;
      try {
        ...
        setError1("Aucun(e) maquilleuses(eurs)) n'a été trouvé dans votre zone.");
      }
    }
  };
    ...
    return(
        <>
            ...
            <div className='row align-items-start justify-content-between first'>
                ...
                <p className='para2'>
                Faites vous belle<br/> 
                par une esthéticienne<br/> 
                qualifiée non loin de vous.
                </p>
                </div>
